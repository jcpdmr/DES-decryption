...
uint64_t permute(const uint64_t &key, const uint16_t *pattern, uint16_t n);
...
void benchmark_parallel_string(const string& correct_plaintext, const string& correct_ciphertext, uint64_t bin_key);
